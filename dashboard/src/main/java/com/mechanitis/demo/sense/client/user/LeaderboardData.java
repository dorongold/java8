...
public class LeaderboardData implements MessageListener<String> {
    ...
    public void onMessage(String twitterHandle) {
        System.out.println("leaderboard message: " + twitterHandle);

        TwitterUser twitterUser = allTwitterUsers.computeIfAbsent(twitterHandle, TwitterUser::new);

        twitterUser.incrementCount();

        List<TwitterUser> topTweeters = allTwitterUsers.values().stream().sorted(Comparator.comparing(TwitterUser::getTweets).reversed()).limit(NUMBER_OF_LEADERS).collect(Collectors.toList());
