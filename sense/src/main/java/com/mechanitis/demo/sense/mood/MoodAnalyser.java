...
public class MoodAnalyser {
    ...
    public static MoodyMessage analyseMood(String fullMessage) {
        System.out.println("analyzing message: " + fullMessage);
        String[] wordsInMessage = getTweetMessageFrom(fullMessage).split(" ");
        //TODO: figure out the moods in this message
        Set<Mood> moods = Stream.of(wordsInMessage).map(String::toLowerCase).map(WORD_TO_MOOD::get).filter(mood -> mood != null).collect(Collectors.toSet());
